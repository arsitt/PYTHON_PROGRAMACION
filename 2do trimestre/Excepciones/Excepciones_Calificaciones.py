...
class Calificaciones():
    ...
    @classmethod
    def Crea_CSV(cls, obj_csv):
        with open(obj_csv, mode = 'r') as alumno:
            lector = csv.DictReader(alumno)
            line_count = 0
            for row in lector:
                if line_count == 0:
                    print(f'Las lineas son {", ".join(row)}')
                else:
                    print(f"{row}")
                print(row)
    ...
    def valida_notas(lista_notas):
        ...
        if lista_notas == []:
            # valido = False
            raise NotasInvalidasError("La lista de notas esta vacía.")
...
print(Calificaciones.Crea_CSV(r"C:\Users\Mañana\Desktop\Alejandro Rodriguez Curso 21-22\PROGRAMACIONGIT\PYTHON_PROGRAMACION\2do trimestre\Excepciones\alumno.csv"))
